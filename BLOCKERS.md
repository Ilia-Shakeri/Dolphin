...
- SRC-002 is blocked on user authorization for one exact commit/release artifact. Automatic commit is forbidden. The settled worktree manifest now exactly covers 134 paths; a later reference-bound regeneration is still required. Until that reference and proof are reviewed, there is no deployable rollback artifact or State A claim. State B may use only `production candidate; external verification pending` after every repository gate and final audit pass.
...
| EXT-007 | BLOCKED_EXTERNAL | Real reviewed application/scanner digests, clean isolated scan host, restricted evidence root, public TLS target/client, and security reviewer | Hashed Python lock and Docker build digest/interpreter/platform gates exist. Compose interpolation proves only nonempty values; the required validator/release flow rejects mutable or malformed refs. No real image, SBOM, advisory/container scan, or TLS scan is proven | Run `C-SCANS`; evidence binds lock hash, tool databases/versions, real digests, SBOM, scans, TLS result, review, and sealed anchor to the exact release |
...
`C-SCANS` - after EXT-007 records the exact commit, application and five scanner digests, isolated scan host, restricted external evidence root, public hostname/client, owners, retention, and reviewer, execute every command in `docs/ops/SECURITY_SCANS.md` from its clean approved checkout. Retain the reduced source report, locked-dependency report, SBOM, image findings, TLS reports, tool/database metadata, exit codes, reviewer dispositions, integrity manifest, and out-of-band anchor. Repository parsing cannot close this external gate.
...
| SRC-002 | BLOCKED_DECISION | Explicit user authorization for one immutable commit or release artifact | Run `C-REF`, regenerate the exact source manifest against that reference, review staged/ref paths, then run `C-REPO` from that artifact |
...
| EXT-007 | BLOCKED_EXTERNAL | Reviewed application/scanner digests, isolated scan host, restricted evidence root, public TLS target/client, owners, retention, and reviewer | Run `C-SCANS`; record exact artifacts, lock hash, tool/database versions, UTC times, exit codes, finding dispositions, report hashes, and sealed out-of-band anchor |
