...
RESTORE_NAME_PATTERN = re.compile(r"^restore_dolphin_[a-f0-9]{32}$")
...
def build_postgres_restore_database(environment):
    ...
    restore database.
    """
    if environment.get("DOLPHIN_PG_RESTORE") != "1":
        raise ImproperlyConfigured(
            "DOLPHIN_PG_RESTORE=1 is required for the isolated PostgreSQL restore settings."
        )

    token = environment.get("DOLPHIN_PG_RESTORE_TOKEN", "")
    if not TOKEN_PATTERN.fullmatch(token):
        raise ImproperlyConfigured(
            "DOLPHIN_PG_RESTORE_TOKEN must be a random 32-character lowercase hex value."
        )

    host = environment.get("DOLPHIN_PG_RESTORE_HOST", "")
    if host != "127.0.0.1":
        raise ImproperlyConfigured("PostgreSQL restore proof must use the IPv4 loopback host.")

    try:
        port = int(environment.get("DOLPHIN_PG_RESTORE_PORT", ""))
    except ValueError as exc:
        raise ImproperlyConfigured(
            "DOLPHIN_PG_RESTORE_PORT must be a high local port."
        ) from exc
    ...
    expected_name = f"restore_dolphin_{token}"
    name = environment.get("DOLPHIN_PG_RESTORE_NAME")
    ...
    expected_user = f"dolphin_app_{token}"
    if environment.get("DOLPHIN_PG_RESTORE_USER") != expected_user:
        raise ImproperlyConfigured(
            "PostgreSQL restore proof must use the ordinary application login."
        )

    password = environment.get("DOLPHIN_PG_RESTORE_PASSWORD", "")
    if len(password) < 16:
        raise ImproperlyConfigured(
            "DOLPHIN_PG_RESTORE_PASSWORD must be an isolated non-empty proof secret."
        )
