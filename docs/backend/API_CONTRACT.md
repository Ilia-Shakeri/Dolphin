...
- `GET exports/user-performance.xlsx`: returns the same scoped rows, four-value summary, and filters as an XLSX workbook. Content type is `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`; filename is `kariz-user-performance.xlsx`.
