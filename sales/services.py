...
OPERATIONAL_WRITERS = {User.Role.SALES_AGENT, User.Role.SALES_MANAGER, User.Role.PLATFORM_ADMIN}
...
def _lock_active_actor(actor):
    ...
    if locked is None or locked.role not in VALID_ROLES:
...
def create_customer_phone(*, actor, customer, raw_phone, label="", is_primary=False, is_active=True):
    ...
    try:
        ...
        raise BusinessRuleError({"raw_phone": "Active phone or primary-phone constraint failed."}) from exc
...
def create_product(*, actor, **data):
    ...
    try:
        ...
        raise BusinessRuleError({"sku": "SKU already exists or product data is invalid."}) from exc
...
def reassign_lead(*, actor, lead, to_user, reason=""):
    ...
    if not target.is_active or target.role != User.Role.SALES_AGENT:
        raise BusinessRuleError({"to_user": "Target must be an active Sales Agent."})
    locked = Lead.objects.select_for_update().get(pk=lead.pk)
    previous = locked.assigned_to
    if previous == target:
        raise BusinessRuleError({"to_user": "Lead is already assigned to this user."})
...
def mark_sale(*, actor, lead, product=None, quantity=1, total_amount=None, **data):
    ...
    if product:
        ...
        if not product.is_active:
            raise BusinessRuleError({"product": "Product is inactive."})
...
def cancel_sale(*, actor, sale, reason=""):
    ...
    if locked.status == Sale.Status.CANCELLED:
        raise BusinessRuleError({"status": "Sale is already cancelled."})
...
def deactivate_customer(*, actor, customer):
    ...
    if not customer.is_active:
        raise BusinessRuleError({"is_active": "Customer is already inactive."})
...
def deactivate_product(*, actor, product):
    ...
    product = Product.objects.select_for_update().get(pk=product.pk)
    if not product.is_active:
        raise BusinessRuleError({"is_active": "Product is already inactive."})
