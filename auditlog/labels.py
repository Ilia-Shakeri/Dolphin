...
OPERATION_LABELS = {
    ...
    "sms_template.created": "ساخت الگوی پیامک",
    "sms_template.deleted": "حذف الگوی پیامک",
    ...
    "dashboard_settings.updated": "تغییر چیدمان داشبورد",
    # پشتیبان‌گیری (common/backups.py). هر سه رویداد عمداً ثبت
    # می‌شوند: دانلود یک نسخه یعنی کل پایگاه داده از سرور خارج شده،
    # و بازگردانی تنها عملیاتی است که تاریخچهٔ فقط-افزودنی را بازنویسی
    # می‌کند — دقیقاً همان دو چیزی که بعداً کسی دنبالشان می‌گردد.
    "backup.create": "درخواست پشتیبان‌گیری",
    "backup.downloaded": "دانلود پشتیبان",
    "backup.restore": "درخواست بازگردانی پایگاه داده",
