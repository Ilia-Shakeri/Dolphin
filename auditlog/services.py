...
_QUANTITY_KEYS = ("quantity", "resulting_quantity", "item_count", "installment_count", "size_bytes")
...
# Backup and restore (common/backups.py). Every one of these is generated by
# this product, never typed: a spool token, a SHA-256, and an archive name in
# the one shape `scripts/backup-postgres.sh` produces. The operator's own
# filename for an upload is deliberately *not* here — it is free text, which
# is the exact thing this allowlist exists to keep out of a payload readable
# at a lower privilege than the row it describes. It lives on
# `common.models.BackupJob.original_filename` instead.
_HEX_KEYS = {"token": 32, "sha256": 64}
_ARCHIVE_NAME = re.compile(r"^dolphin-pg-[0-9]{8}T[0-9]{6}Z-[0-9a-f]{32}\.dump$")
...
def _clean_changes(changes):
    ...
    for key, width in _HEX_KEYS.items():
        value = changes.get(key)
        if isinstance(value, str) and re.fullmatch(f"[0-9a-f]{{{width}}}", value):
            cleaned[key] = value
    archive = changes.get("archive")
    if isinstance(archive, str) and _ARCHIVE_NAME.fullmatch(archive):
        cleaned["archive"] = archive
