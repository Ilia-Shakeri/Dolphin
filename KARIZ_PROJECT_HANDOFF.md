...
- Active phase: Client-1 provider-neutral inbound SMS storage/reporting; repository portion `DONE_LOCAL`. Live provider adapter/webhook is `BLOCKED_EXTERNAL` and is not claimed.
- Git HEAD: `14e3454b9084677ba3df778a8962dea596ff7e15`.
- Git status: dirty working tree containing only the uncommitted first-party SMS slice and its contract/test/handoff updates; no commit created. Exact final porcelain status is recorded in the latest checkpoint below.
- Implemented core: session login/logout/current profile; backend capability map; shared role-aware dashboard/navigation; controlled user custody; Customer/CustomerPhone; Lead assignment/history and agent work queue; manual Interaction/follow-up; Product; operational Sale; internal SalesDocument/postal history/report; additive AfterSalesRequest; scoped ActivityLog; authoritative performance JSON/UI/XLSX; and provider-neutral append-only InboundSMS storage plus company SMS date/hour report and drill-down. The SMS model stores no message body and has no mutation or public ingest API.
...
- Resolved decisions: prior role/terminology/document/after-sales/performance decisions plus this bounded SMS contract. Only normalized inbound envelopes are stored; body policy is fixed `not_retained`; `(provider_code, external_message_id)` is the canonical idempotency key; conflicting replay fails closed; metadata is scalar/allowlisted/bounded; Customer matches one active normalized phone and Lead links only when exactly one exists; report grouping is `Asia/Tehran`; aggregate and drill-down share one backend selector/filter. Older claims that all SMS repository work is open are historical/superseded; live provider work remains blocked.
- Still open: official SMS provider identity/auth/signature/replay/payload/short-code/retry docs, sandbox credentials and verified provider delivery; contact-status derivation; P&L/receivable sources; new KPI formulas; exact after-sales status/reopen/SLA; governance/seat/UAT; and external runtime proof. No webhook, live provider, outbound SMS, raw payload/body, accounting Invoice, Payment, Finance, Files, Inventory, Opportunity, Team, or profit calculation was added.
- Verification truth: SMS/backend/infrastructure/browser focused suite `43/43` with `1` intentional PostgreSQL-only skip; dedicated real-browser SMS authorization/report journey `2/2`; full Django suite `331` run, `324` pass and `7` intentional PostgreSQL-only skips. OpenAPI fail-on-warn, Django check, migration drift, JavaScript syntax, branding `226`, collectstatic dry-run, PowerShell parse, Bash syntax, and diff whitespace pass. Native isolated PostgreSQL execution remains unavailable on this host; its static contracts pass and its new race proof is correctly PostgreSQL-only.
- Exact next phase: obtain and approve every item in `docs/backend/SMS_PROVIDER_ADAPTER_REQUIREMENTS.md`, then implement one named authenticated adapter and sandbox proof. Until then expose no webhook and make no live-integration claim.
