...
- Active phase: `FINAL_WAVE_LOW` module 1, Product Category and final Product form; `DONE` in the repository and real local browser. Production/runtime proof remains external. Module 2 Inventory is not started.
- Git HEAD: `1c1f168d3731996415fcb3914a386c6928048756`.
- Git status after module 1: dirty by this requested slice only, with `29` tracked modified paths and `3` untracked porcelain entries (`common/templates/common/product_categories/`, `sales/migrations/0013_product_barcode_product_brand_productcategory_and_more.py`, `sales/tests/test_product_categories.py`), total `32`; no commit created.
- Implemented core: session login/logout/current profile; backend capability map; shared role-aware dashboard/navigation; controlled user custody; Customer/CustomerPhone; Lead assignment/history and agent work queue; manual Interaction/follow-up; flat ProductCategory plus expanded Product Category/brand/barcode catalog; operational Sale; internal SalesDocument/postal history/report; additive AfterSalesRequest; scoped ActivityLog; authoritative performance JSON/UI/XLSX; and provider-neutral append-only InboundSMS storage plus company SMS date/hour report and drill-down. The SMS model stores no message body and has no mutation or public ingest API.
- Current role map: four role codes stay fixed: `sales_agent` = `بازاریاب (کال سنتر)`; `sales_manager` = `مدیر فروشگاه`; `company_it` = `مدیر فنی مشتری`; `platform_admin` = `مدیر پلتفرم`. Sales Agent workstream is bounded to `sales` or `after_sales`; elevated roles must remain `sales`. This is not a fifth role or dynamic permission builder. `sms.company` belongs only to Sales Manager, Company IT, and Platform Admin; Sales Agent cannot see SMS navigation, counts, filters, drill-down, or direct rows.
...
- Resolved decisions: prior role/terminology/document/after-sales/performance/SMS decisions plus the bounded Product Category/final form contract below. Category is flat, Product owns zero or one optional Category, legacy mapping is null, normalized Category name and nonblank barcode are globally unique, and lifecycle is non-destructive. Older statements that Product Category/form work is absent or blocked are historical/superseded.
- Still open: official SMS provider material; contact-status derivation; Inventory/finance/storage/search/import/integration contracts; new KPI formulas; exact after-sales status/reopen/SLA; governance/seat/UAT; and external runtime proof. Inventory unit, location/warehouse, opening balance, movement types, reservation, negative-stock, adjustment, reversal, and migration semantics remain genuinely unresolved. Purchase cost, multi-price, discount, finance, storage, integration, automation, and PWA semantics stay separate.
- Verification truth: focused Product Category/Product/security/infrastructure suite `46/46`; affected workflow suite `58/58`; OpenAPI plus system/API focused suite `31/31`; dedicated real Chrome manager-to-agent Category/Product journey `1/1`; full Django suite `338` run with `331` pass and `7` intentional PostgreSQL-only skips. Django check, migration drift, OpenAPI fail-on-warn, JavaScript syntax, branding `228`, collectstatic dry-run, PowerShell parse, Git Bash syntax, and diff whitespace pass. Native isolated PostgreSQL execution remains unavailable on this host; its ACL/schema/restore static contracts pass.
- Exact next phase: `FINAL_WAVE_LOW` module 2 Inventory/stock-movement contract checkpoint. Decide exact unit, warehouse/location, opening balance, movement/event types, reservation, negative-stock, adjustment, reversal, concurrency, and legacy-data semantics before its own migration. Do not add purchase cost, multi-price, discount, quotation, accounting Invoice, Payment, or files to the Inventory migration.
