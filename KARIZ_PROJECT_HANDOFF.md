...
### تنها شکاف فارسی‌بودن که باز مانده — و چرا عمدا باز مانده است

تاریخ‌ها میلادی نمایش داده می‌شوند و ورودی‌های `datetime-local` قالب محلی مرورگر را نشان می‌دهند (روی مرورگر با locale انگلیسی: `mm/dd/yyyy`). این **تصمیم باز ثبت‌شده** است، نه سهو: `docs/backend/OPEN_BUSINESS_DECISIONS.md` («آیا تاریخ جلالی نمایش داده شود، میلادی، یا هر دو؟») و `BIZ-007`. انتخاب تقویم روی یک سند مالی یک تصمیم کسب‌وکاری واقعی است (کدام تقویم روی فاکتور معتبر است، سال مالی، سررسید)، پس طبق `CLAUDE.md` حدس زده نشد. **اکنون این تصمیم برای کاربر قابل‌مشاهده است** و پیش از تحویل به مشتری باید بسته شود.
